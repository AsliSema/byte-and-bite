...
/* Controller for getting a dish by ID
     ...
     access: public 
*/
const getDishById = asyncHandler(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const order of orders) {
      for (const dish of order.orderItems) {
        if (
          dish.product.toString() === req.params.dishID &&
    ...
    if (!canReview) {
      return next(
        new ApiError(StatusCodes.BAD_REQUEST, 'You must buy this dish first!')
      );
    }
...
/**
 * Delete a review for a dish
 * @route DELETE /api/dish/review/:reviewID
 * @access Private Customer
 */
const deleteReview = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.user?.role === 'admin') {
      ...
      deletedReview = await Review.findOneAndDelete({
        $and: [{ user: req.user?._id }, { _id: req.params.reviewID }],
      });
    }

    if (!deletedReview) {
      return next(
        new ApiError(StatusCodes.BAD_REQUEST, 'You can not delete this review!')
      );
    }
 ...
 * @route PUT /api/dish/review/:reviewID
 * @access Private Customer
 */
const updateReview = asyncHandler(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!updatedReview) {
      return next(
        new ApiError(StatusCodes.BAD_REQUEST, 'You can not update this review!')
      );
    }
